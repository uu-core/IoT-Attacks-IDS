...
for(var i = 29; i &gt; 0; i--) {
  /* Start attack */
  setBool(attacker, 'network_attacks_rpl_vna', true);
  log.log("Version number attack from " + attacker.getID() + "!\n");
  sim.getEventCentral().logEvent("attack", "version number:" + attacker.getID());

  GENERATE_MSG(60000, "continue")
  YIELD_THEN_WAIT_UNTIL(msg.equals("continue"));

  /* Stop attack */
  setBool(attacker, 'network_attacks_rpl_vna', false);
  log.log("Stopping - Version number attack from " + attacker.getID() + "!\n");
  sim.getEventCentral().logEvent("stop-attack", "version number:" + attacker.getID());

  /* Wait i minutes */
  GENERATE_MSG(i * 60000, "continue");
  YIELD_THEN_WAIT_UNTIL(msg.equals("continue"));
}
