...
def generated_dirs(scenario_dir: Path):
    result = []
...
def scenario_file(project: Path, start: int, run: int) -> Path:
    return (
        ...
        / "15"
        / f"start_{start}"
        / f"dis_flooding-oo-15-start-{start}-run-{run:02d}.csc"
    )
...
def generate_features(project: Path, dest: Path):
    log = dest / "runner.log"
...
def run_one(start: int, run: int) -> str:
    ...
    label = f"DIS on-off start={start} run={run:02d}"
    ...
    # Remove temporary Cooja output directories left in the scenario folder.
    ...
    try:
        while time.time() < deadline:
            dirs = generated_dirs(scenario_dir)

            if dirs:
                generated = dirs[-1]

                if has_test_ok(generated / "script.log"):
                    success = True
                    break
    ...
    # These files are huge and are not needed for feature generation.
    for name in (
        "radio-log.pcap",
        "radio-medium.log",
    ):
    ...
    generate_features(project, dest)

    return f"DONE {label}"


def run_start(start: int):
    results = []

    for run in RUNS:
        message = run_one(start, run)
...
def main():
    ...
    print(
        "Running 30 DIS Flooding on-off controlled simulations...",
        flush=True,
    )

    failures = []

    # Two activation-time groups in parallel, same strategy as old safe runner.
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = {
            executor.submit(run_start, start): start
            for start in STARTS
        }
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                message = f"FAILED start={start}: {exc}"
    ...
    print()
    print("All 30 DIS Flooding on-off runs completed.")
