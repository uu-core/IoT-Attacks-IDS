...
def run_one(start: int, run: int) -> str:
    ...
    label = f"Local Repair on-off start={start} run={run:02d}"
    ...
    # Delete temporary Cooja dt-* output directories.
    ...
    # Remove very large files not required by the ML pipeline.
    for name in (
        "radio-log.pcap",
        "radio-medium.log",
    ):
...
def main() -> None:
    print(
        "Running 30 Local Repair on-off controlled simulations...",
        flush=True,
    )
    ...
    print()
    print(
        "All 30 Local Repair on-off runs completed."
    )
