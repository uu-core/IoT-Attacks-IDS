...
FEATURE_SETS = {
    # Two-feature routing-state baseline.
    "rank_only": [
        "rank_mean", "rank_std",
    ],

    # Same routing-state baseline augmented with four interval TX/RX features.
    "rank_plus_interval_txrx": [
        "rank_mean", "rank_std",
        "delta_txraw_mean", "delta_txraw_std",
        "delta_rxraw_mean", "delta_rxraw_std",
    ],

    # Six-feature reduced RPL baseline:
    # rank plus interval DAO activity and total received traffic.
    "reduced_rpl": [
        "rank_mean", "rank_std",
    ...
    # Reduced RPL augmented with four interval TX/RX features.
    "reduced_rpl_plus_interval_txrx": [
        "rank_mean", "rank_std",
        "delta_diar_mean", "delta_diar_std",
        "delta_tots_mean", "delta_tots_std",
        "delta_txraw_mean", "delta_txraw_std",
...
def make_plots(results: pd.DataFrame, out_dir: Path) -> None:
    ...
    summary.to_csv(
        out_dir / "reduced_rpl_summary.csv",
        index=False,
    )
    ...
    overall.to_csv(
        out_dir / "reduced_rpl_overall.csv",
        index=False,
    )
    ...
    gains.to_csv(
        out_dir / "reduced_rpl_paired_gains_all_runs.csv",
        index=False,
    )
    ...
    gain_summary.to_csv(
        out_dir / "reduced_rpl_paired_gains_summary.csv",
        index=False,
    )
    ...
    gain_overall.to_csv(
        out_dir / "reduced_rpl_paired_gains_by_attack.csv",
        index=False,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path.home() / "ids-WPLR/temporal_validation/reduced_rpl_results",
    )
        ...
        for held_out_start in START_TIMES:
            ...
            for feature_name, columns in FEATURE_SETS.items():
                ...
                for seed in SEEDS:
                    ...
                    pd.DataFrame(records).to_csv(args.output_dir / "reduced_rpl_all_runs.csv", index=False)

    results = pd.DataFrame(records)
    make_plots(results, args.output_dir)
    print("\nCompleted.")
    print(args.output_dir / "reduced_rpl_all_runs.csv")
    print(args.output_dir / "reduced_rpl_summary.csv")
    print(args.output_dir / "reduced_rpl_overall.csv")
