...
FEATURE_SETS = {
    ...
    # Fair RPL-10 baseline from the nested RPL experiment.
    "rpl_10": [
        "rank_mean", "rank_std",
...
def make_plots(results: pd.DataFrame, out_dir: Path) -> None:
    ...
    summary.to_csv(
        out_dir / "tx_rx_ablation_summary.csv",
        index=False,
    )
    ...
    overall.to_csv(
        out_dir / "tx_rx_ablation_overall.csv",
        index=False,
    )
    ...
    comparisons = [
        ...
        (
            "rpl_10",
            "rpl_10_plus_txrx",
            "rpl_10_plus_txrx_gain",
        ),
    ]

    gain_frames: list[pd.DataFrame] = []

    for baseline, augmented, comparison in comparisons:
    ...
    gains.to_csv(
        out_dir / "tx_rx_ablation_paired_gains_all_runs.csv",
        index=False,
    )
    ...
    by_start.to_csv(
        out_dir / "tx_rx_ablation_paired_gains_by_start.csv",
        index=False,
    )
    ...
    by_attack.to_csv(
        out_dir / "tx_rx_ablation_paired_gains_by_attack.csv",
        index=False,
    )
    ...
    radio_only.to_csv(
        out_dir / "tx_rx_radio_only_by_attack.csv",
        index=False,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path.home() / "ids-WPLR/temporal_validation/tx_rx_ablation_results",
    )
        ...
        for held_out_start in START_TIMES:
            ...
            for feature_name, columns in FEATURE_SETS.items():
                ...
                for seed in SEEDS:
                    ...
                    pd.DataFrame(records).to_csv(args.output_dir / "tx_rx_ablation_all_runs.csv", index=False)

    results = pd.DataFrame(records)
    make_plots(results, args.output_dir)
    print("\nCompleted.")
    print(args.output_dir / "tx_rx_ablation_all_runs.csv")
    print(args.output_dir / "tx_rx_ablation_summary.csv")
    print(args.output_dir / "tx_rx_ablation_overall.csv")
    print(args.output_dir / "tx_rx_ablation_f1.png")
