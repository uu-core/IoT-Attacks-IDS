...
def process_folders(root_folder):
    ...
    for size_folder in os.listdir(root_folder):
        ...
        for var_folder in os.listdir(size_path):
            ...
            # Loop over each subfolder inside varY (dt-xxxx)
            for run_folder in os.listdir(var_path):
                run_path = os.path.join(var_path, run_folder)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python3 gen_csv.py folder1,folder2,...")
        sys.exit(1)
    ...
    base_path = os.path.join(
        SCRIPT_DIR,
        "../applications/example-attacks/scenarios_output"
    )
