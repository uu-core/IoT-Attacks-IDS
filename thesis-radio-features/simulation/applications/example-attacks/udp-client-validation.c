...
#define NORMAL_TRAFFIC_MAX_INTERVAL (180 * CLOCK_SECOND)
#define SHOW_ENERGEST (1 && ENERGEST_CONF_ON)

static struct simple_udp_connection udp_conn;
static struct simple_udp_connection node_to_node_conn;

PROCESS(udp_client_process, "UDP client");
PROCESS(normal_udp_traffic_process, "Normal UDP traffic");
AUTOSTART_PROCESSES(&udp_client_process, &normal_udp_traffic_process);
...
static char* get_random_node_ip(uip_ipaddr_t *dest_ipaddr) {
  uiplib_ipaddrconv(node_ip_list[0], dest_ipaddr);
  return node_ip_list[0];
}
...
PROCESS_THREAD(udp_client_process, ev, data)
{
  ...
  while(1) {
    ...
    if(NETSTACK_ROUTING.node_is_reachable() &&
      ...
      /*
       * Flush Energest before logging so TX/RX are captured as precise
       * cumulative raw counters rather than whole seconds. The current
       * application packet is transmitted immediately after this log line,
       * so its radio cost is included in the next observation.
       */
      energest_flush();
      tx_raw = energest_type_time(ENERGEST_TYPE_TRANSMIT);
      rx_raw = energest_type_time(ENERGEST_TYPE_LISTEN);
      ...
      LOG_INFO_(",diar:%"PRIu32",tots:%"PRIu32",txraw:%"PRIu64",rxraw:%"PRIu64"\n",
